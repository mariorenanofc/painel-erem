...
export default function PortalLogin() {
  ...
  // --- ESTADOS DA RECUPERAÇÃO DE MATRÍCULA ---
  ...
  const [resultadoRecuperacao, setResultadoRecuperacao] = useState<{ matricula: string; nome: string } | null>(null);
  ...
  // ==========================================
  // FUNÇÃO DE RECUPERAR MATRÍCULA
  // ==========================================
  const buscarMatricula = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nomeRecuperacao.trim() || !dataNascRecuperacao.trim()) {
      setErroRecuperacao("Preencha o nome e a data de nascimento."); return;
    }
    setBuscandoMatricula(true); setErroRecuperacao(""); setResultadoRecuperacao(null);

    // Converte a data do input (YYYY-MM-DD) para o padrão brasileiro (DD/MM/YYYY) para busca no backend
    ...
    try {
      const res = await fetch(GOOGLE_API_URL, {
        ...
        body: JSON.stringify({ action: "recuperar_matricula", nome: nomeRecuperacao, dataNasc: dataFormatada })
      });
      const data = await res.json();
      
      if (data.status === "sucesso") {
        setResultadoRecuperacao({ matricula: data.matricula, nome: data.nomeReal });
        
        // Mágica de UX: Já preenche o form principal de login para poupar trabalho do aluno!
    ...
    return (
      <div className="min-h-screen bg-red-600 flex flex-col items-center justify-center p-4 text-center text-white animate-in fade-in duration-500">
        ...
        <p className="text-xl max-w-lg font-medium leading-relaxed mb-8">
          Olá, <strong>{nomeBloqueado}</strong>. Identificamos uma infração grave às regras do projeto Trilha Tech. 
          O seu acesso à plataforma, missões e ranking foi temporariamente suspenso.
        </p>
        <div className="bg-red-800/50 p-6 rounded-2xl max-w-md border border-red-500 shadow-xl mb-8">
          <p className="font-bold text-red-100 uppercase tracking-widest text-sm mb-2">Próximos Passos</p>
          <p className="text-sm">Por favor, compareça à sala da coordenação do Trilha Tech presencialmente para conversar com o Tutor e a Gestão Pedagógica.</p>
        </div>
        <button 
          onClick={() => { setAcessoNegado(false); setMatricula(""); setDataNasc(""); }}
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 relative z-10">
        <div className="text-center mb-8">
          <div className="bg-blue-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 text-4xl">🚀</div>
          <h1 className="text-2xl font-black text-slate-800">Portal Trilha Tech</h1>
          <p className="text-slate-500 text-sm mt-1">Acesse sua área de aluno</p>
        </div>
        ...
        <div className="mt-6 text-center border-t border-slate-100 pt-5">
          <button 
            type="button"
            onClick={() => { setModalRecuperacaoAberto(true); setResultadoRecuperacao(null); setErroRecuperacao(""); setNomeRecuperacao(""); setDataNascRecuperacao(""); }}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col p-6 text-center">
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-black text-xl text-slate-800 flex items-center gap-2"><span>🔍</span> Consultar Matrícula</h2>
              <button onClick={() => setModalRecuperacaoAberto(false)} className="text-3xl leading-none text-slate-400 hover:text-slate-600">&times;</button>
            </div>

            {resultadoRecuperacao ? (
              <div className="py-6 animate-in zoom-in">
                <div className="text-5xl mb-4">🎉</div>
                <h3 className="font-bold text-slate-800 mb-2">Matrícula Encontrada!</h3>
                <p className="text-sm text-slate-500 mb-6">Olá, {resultadoRecuperacao.nome}. Anote o seu número de acesso:</p>
                
                <div className="bg-blue-50 border-2 border-dashed border-blue-300 rounded-xl p-4 mb-6">
                  <span className="text-3xl font-black text-blue-700 font-mono tracking-widest">{resultadoRecuperacao.matricula}</span>
                </div>
              ...
              <>
                <p className="text-sm text-slate-500 mb-6 text-left">
                  Preencha os seus dados exatamente como estão na secretaria da escola para localizar o seu registro.
                </p>

                {erroRecuperacao && <div className="bg-red-50 text-red-600 p-3 rounded-lg text-xs mb-4 border border-red-200 text-left font-bold">{erroRecuperacao}</div>}

                <form onSubmit={buscarMatricula} className="space-y-4 text-left">
                  <div>
                  ...
                  >
                    {buscandoMatricula ? "Consultando Banco de Dados..." : "Consultar Matrícula"}
                  </button>
                </form>
              </>
            )}
