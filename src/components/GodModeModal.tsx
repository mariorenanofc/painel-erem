...
export default function GodModeModal({ onClose, onSuccess }: GodModeModalProps) {
  ...
  useEffect(() => {
    const buscarAlunos = async () => {
      try {
        const res = await fetch(GOOGLE_API_URL, {
          ...
          body: JSON.stringify({ action: "listar_alunos_godmode" }),
        });
    ...
    if (GOOGLE_API_URL) buscarAlunos();
  }, []);

  const handleInjetar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!matriculaSelecionada || quantidadeXP === "" || Number(quantidadeXP) === 0 || !motivo) {
      return alert("Preencha todos os campos e use um valor diferente de zero!");
    }
    ...
    try {
      const res = await fetch(GOOGLE_API_URL, {
        ...
        body: JSON.stringify({
          ...
          matriculaAlvo: matriculaSelecionada,
          quantidadeXP: Number(quantidadeXP),
          motivo: motivo,
        }),
      });
      const data = await res.json();
      ...
      if (data.status === "sucesso") {
        if (Number(quantidadeXP) > 0) {
            confetti({ particleCount: 150, spread: 80, colors: ["#fbbf24", "#f59e0b", "#fff"] });
        }
        alert("⚡ " + data.mensagem);
        onSuccess();
        onClose();
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col relative border-4 border-amber-400">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 p-6 relative overflow-hidden">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 text-3xl leading-none text-white hover:text-amber-400 z-10">&times;</button>
        ...
        <div className="p-6 bg-slate-50">
          {carregando ? (
             <div className="flex justify-center py-10"><div className="animate-spin rounded-full h-10 w-10 border-b-4 border-purple-600"></div></div>
          ) : (
            <form onSubmit={handleInjetar} className="space-y-5">
              
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">1. Escolha o Alvo</label>
                <select 
                ...
                >
                  ...
                  {alunos.map(a => (
                    <option key={a.matricula} value={a.matricula}>{a.nome} ({a.turma})</option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 md:col-span-1">
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">2. Quantidade (+ ou -)</label>
                ...
                <div className="col-span-2 md:col-span-1 flex flex-col justify-end pb-2">
                   <p className="text-[10px] text-slate-500 font-bold leading-tight">
                     💡 <span className="text-emerald-600">Positivo (+10)</span> para premiar. <br/>
                     🚨 <span className="text-red-500">Negativo (-100)</span> para punir IA/cola.
                   </p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">3. Motivo da Ação (Aluno vai ver!)</label>
                <input 
                  type="text" 
                  value={motivo} 
              ...
              >
                {injetando ? "Aplicando..." : Number(quantidadeXP) < 0 ? "🚨 APLICAR PUNIÇÃO" : "⚡ INJETAR BÔNUS"}
              </button>
