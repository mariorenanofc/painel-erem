...
export default function TrilhaTable({ alunosFiltrados, atualizandoMatricula, mudarStatus, abrirModalVisualizacao }: TrilhaTableProps) {
  return (
        ...
        <table className="w-full text-left border-collapse min-w-200">
          ...
          <tbody className="text-slate-700 text-sm">
            {alunosFiltrados.length > 0 ? (
                ...
                <tr key={index} className="border-b border-slate-200 hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold uppercase ${
                      ...
                      aluno.statusTrilha === "Inscrito" ? "bg-blue-100 text-blue-700" : "bg-slate-200 text-slate-600"
                    }`}>
                      {aluno.statusTrilha}
                    </span>
                  </td>
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-3">
                      <button onClick={() => abrirModalVisualizacao(aluno)} className="text-xl hover:scale-125 hover:text-blue-600 transition-all" title="Ver Contatos do Aluno">
                        👁️
                      </button>
                      ...
                      {atualizandoMatricula === aluno.matricula ? (
                        ...
                        <select className="bg-white border border-slate-300 text-slate-700 text-xs rounded p-1.5 focus:outline-none focus:border-emerald-500 cursor-pointer shadow-sm w-37.5" value="" onChange={(e) => mudarStatus(aluno.matricula, e.target.value)}>
              ...
              <tr><td colSpan={5} className="p-8 text-center text-slate-500 font-medium">Nenhum aluno encontrado com estes filtros...</td></tr>
            )}
          </tbody>
        </table>
      </div>
