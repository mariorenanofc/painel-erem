...
async function rebuildRankings() {
    ...
    console.log("Reconstruindo rankings baseados na collection entregas com checagem de ATRASO...");
    ...
    entregasSnap.forEach(doc => {
        ...
        // Apenas reconstruir semana e mês atuais (W35 e 08/2026)
        ...
        // Outros indícios de atraso (fallback)
        if (data.feedback && (data.feedback.includes("por Atraso") || data.feedback.includes("Gabarito Liberado"))) {
            ehAtrasado = true;
        }

        const xpNormal = ehAtrasado ? 0 : data.xpGanho;
        const xpAtrasado = ehAtrasado ? data.xpGanho : 0;
    ...
    for (const [semana, alunos] of Object.entries(rankingsSemana)) {
        ...
        batch.set(ref, { alunos }, { merge: false }); // usar false para sobrescrever 100% com os dados calculados corretos
    }
    ...
    for (const [mes, alunos] of Object.entries(rankingsMes)) {
        ...
        batch.set(ref, { alunos }, { merge: false }); // usar false para sobrescrever 100%
    }
